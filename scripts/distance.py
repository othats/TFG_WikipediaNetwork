...
def bfs(start_article, nodes, edges, article_dict):
    ...
    vis = len(visited)
    unvis = len(unvisited)
    percentage = (vis / (vis + unvis)) * 100
    print(f"Number of visited articles: {len(visited)}")
    print(f"Number of unvisited articles: {len(unvisited)}")
    print(f"Percentage of articles that reached Philosophy: {percentage:.2f}%\n")
...
output_file = "output/distances.csv"
